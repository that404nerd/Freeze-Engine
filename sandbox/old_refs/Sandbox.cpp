...
Sandbox::Sandbox() : m_Camera(std::make_shared<Freeze::Camera>((glm::vec4(-20.0f, 20.0f, -12.0f, 12.0f)))), 
  m_Positions(0.0f, 0.0f), m_Velocity(0.0f, 0.0f) {}
...
void Sandbox::OnImGui()
{
  ...
  ImGui::SetNextWindowSize(ImVec2(400.0f, ImGui::GetIO().DisplaySize.y - 50), ImGuiCond_Always);
  ...
  // This will be moved to a seperate file completely. Not planning on making a editor for something so simple.
  for (auto entity : view) {
    ...
    if (ImGui::TreeNodeEx(label.c_str(), ImGuiTreeNodeFlags_DefaultOpen)) {
      ImGui::Text("Positions: %f, %f", physComp.Positions.x, physComp.Positions.y);  
      ImGui::Text("Rotation %f", physComp.Rotation);  
